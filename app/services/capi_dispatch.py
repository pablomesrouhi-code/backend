...
TIKTOK_PURCHASE = "CompletePayment"
...
def _build_user_match(
    *,
    ...
    if phone_hash:
        ...
        snap_user["sha256_phone_number"] = phone_hash
...
async def send_meta_capi_event(
    *,
    ...
    if not pixel_id or not token:
        logger.info("[capi] meta skipped — META_PIXEL_ID or META_CAPI_ACCESS_TOKEN missing")
        return
...
async def send_tiktok_capi_event(
    *,
    ...
    if not pixel_code or not token:
        logger.info("[capi] tiktok skipped — TIKTOK_PIXEL_CODE or TIKTOK_ACCESS_TOKEN missing")
        return
...
async def send_snap_capi_event(
    *,
    ...
    if not pixel_id or not token:
        logger.info("[capi] snap skipped — SNAP_PIXEL_ID or SNAP_ACCESS_TOKEN missing")
        return
